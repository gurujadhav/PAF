...
var PAFMcpAgent = class {
  static artifactDir = path2.resolve(process.cwd(), "test-results/mcp-artifacts");
  /**
  ...
  static emitAgentArtifact(payload) {
    ...
    const filePath = path2.join(this.artifactDir, fileName);
    const content = `
...
var VisualValidator = class {
  ...
  async compareScreenshot(snapshotName, target, options = {}) {
    ...
    await expect2(compareTarget).toHaveScreenshot(`${snapshotName}.png`, {
      threshold,
      maxDiffPixels,
      mask: maskedLocators
    });
...
var AccessibilityValidator = class _AccessibilityValidator {
  ...
  page;
  static reportDir = path3.resolve(process.cwd(), "test-results/accessibility");
  /**
...
` + summaryList.map((s) => `   - [${s.impact?.toUpperCase()}] ${s.id}: ${s.help} (${s.nodesCount} nodes affected) -> ${s.helpUrl}`).join("\n")
      ...
      expect3(violations, `Page "${pageName}" has ${violations.length} accessibility violation(s)`).toEqual([]);
    } else {
  ...
  saveAccessibilityReport(pageName, results) {
    if (!fs2.existsSync(_AccessibilityValidator.reportDir)) {
      fs2.mkdirSync(_AccessibilityValidator.reportDir, { recursive: true });
    }
    const fileName = `a11y-report-${pageName.replace(/[^a-z0-9]/gi, "_")}-${Date.now()}.json`;
    const filePath = path3.join(_AccessibilityValidator.reportDir, fileName);
    fs2.writeFileSync(filePath, JSON.stringify(results, null, 2), "utf-8");
...
var createPAFConfig = (overrides = {}) => {
  ...
  const localSetup = path4.resolve(cwd, "src/core/global.setup.ts");
  const localTeardown = path4.resolve(cwd, "src/core/global.teardown.ts");
  const config = {
    testDir: "./tests",
    timeout: 60 * 1e3,
    expect: {
...
var ArtifactManager = class {
  static resultsDir = path5.resolve(process.cwd(), "test-results");
  /**
  ...
  static getArtifactsForTest(testTitleSanitized) {
    ...
    if (!fs4.existsSync(this.resultsDir)) {
      return details;
    }
    try {
      const dirs = fs4.readdirSync(this.resultsDir);
      for (const dirName of dirs) {
        if (dirName.toLowerCase().includes(testTitleSanitized.toLowerCase().replace(/[^a-z0-9]/gi, "-"))) {
          const fullDirPath = path5.join(this.resultsDir, dirName);
          if (fs4.lstatSync(fullDirPath).isDirectory()) {
            const files = fs4.readdirSync(fullDirPath);
            for (const file of files) {
              const fullFilePath = path5.join(fullDirPath, file);
